...
WHERE (inducted_by IS NOT NULL OR inducted_by = '' ) AND tbl_induction_register.id = '".$_GET['approved_form']."'")->fetch_object();
...
$image_0= $conn->query("Select * from tbl_site_upload_attachment where induction_id = '".$_GET['approved_form']."' order by image_id");
...
while($row = mysqli_fetch_object($image_0))
{
	...
	foreach ($arr as $index => $value) {
		...
		if($index == 0){
		 ...
		 $pdf->Image($path,10,70,40,25,$image_format_uploads,$path);
		}
		if($index == 1){
		 // $path= "http://localhost/site_admin/API/Uploads/".$value;
		 $pdf->Image($path,135,70,40,25,$image_format_uploads,$path);
		}
		if($index == 2){
		 // $path= "http://localhost/site_admin/API/Uploads/".$value;
		 $pdf->Image($path,10,120,40,25,$image_format_uploads,$path);
		}
		if($index == 3){
		 // $path= "http://localhost/site_admin/API/Uploads/".$value;
		 $pdf->Image($path,135,120,40,25,$image_format_uploads,$path);
		...
		if(count($arr)>=5 AND $index>=4)
		{
				...
				if($index == 4)
				{
						...
						$pdf->Image($path,10,170,40,25,$image_format_uploads,$path);
				...
				$pdf->Image($path,135,170,40,25,$image_format_uploads,$path);
				}
				if($index == 6)
				{
				
			
				$pdf->Image($path,10,210,40,25,$image_format_uploads,$path);
				}
				if($index == 7)
				{
				
			
				$pdf->Image($path,135,210,40,25,$image_format_uploads,$path);
				}
				if($index == 8)
				{
				
			
				$pdf->Image($path,10,260,40,25,$image_format_uploads,$path);
				}
				if($index == 9)
				{
				$pdf->Image($path,135,260,40,25,$image_format_uploads,$path);
				...
				$pdf->Image($path,10,30,40,25,$image_format_uploads,$path);

				}
				if($index == 11)
				{
				
				$pdf->Image($path,135,30,40,25,$image_format_uploads,$path);
...
$pdf->Image($path_sign,10,175,50,40,$image_format);
$pdf->Ln();
$pdf->Ln();
$pdf->Ln();
$pdf->Ln();
...
$pdf->SetTextColor(0,0,0  );
			...
			$value_induction_no= str_pad($test->inducted_by, 4, '0', STR_PAD_LEFT);
			$pdf->SetTextColor(52, 152, 219  );
			$pdf->Cell(40,10,"$value_induction_no",0,0,L);
			$pdf->SetTextColor(0,0,0  );
