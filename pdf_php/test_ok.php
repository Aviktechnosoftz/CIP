...
{
	$id= $_REQUEST['id'];
	 @$main_query= $conn->query("select a.id,a.project_id,a.subject,a.instructions,a.today_date,a.emp_id,a.distribution_id as distribution,a.employee_id,a.issued_by,a.req_date,a.attachments,b.company_name,b.phone_no as emp_phone,b.email_add as emp_email,c.given_name,c.surname,c.email_add from tbl_instruction a inner join tbl_employer b on a.emp_id = b.id inner join tbl_employee c on a.employee_id = c.id where a.id='".$id."'")->fetch_object();
...
$employee_name= $conn->query("select given_name,surname,email_add from tbl_employee where id IN (".$main_query->employee_id.")");
        ...
        $str= $rows->given_name." ".$rows->surname." (".$rows->email_add. ") ,".$str;
     }
     	$str2= rtrim($str,",");
...
$pdf->SetTextColor(52, 152, 219  );
// $pdf->setFillColor(28, 40, 51  );
$pdf->Cell(46,10,"Uploaded Attachments:",0,1,L);

$pdf->SetFont('Arial','B',8);
$pdf->SetTextColor(63, 122, 240  );
foreach ($img_arr as $key => $value) {

$pdf->Cell(70,10,"Document Link:  ".$value,0,1,L,"","../API/Uploads/$value");
}
