...
//print_r($_SESSION);die();
  ...
  //$main_query= $conn->query("select *,date(created) as dates,checklist_udid  from tbl_sm_report_filled Where project_id ='".$user."' AND is_submitted = '0'  AND is_deleted = '0' AND date(created)");

  
$main_query= $conn->query("select tbl_sm_report_action.action,tbl_sm_report_filled.checklist_udid,date(tbl_sm_report_filled.created) as created from tbl_sm_report_action inner join tbl_sm_report_filled on tbl_sm_report_filled.checklist_udid = tbl_sm_report_action.checklist_udid where  tbl_sm_report_action.checklist_udid   NOT IN (select `checklist_udid` from tbl_sm_report_action where `action` = 2 group by checklist_udid) AND tbl_sm_report_filled.is_submitted='1'  AND tbl_sm_report_filled.project_id='".$_SESSION['admin']."'  group by tbl_sm_report_action.checklist_udid");
        ...
        <table  class="table table-hover" style= "border: 1px solid rgba(128, 128, 128, 0.57);"">
          ...
          <tr style="background-color: #f5f5f5; font-size: 15px">         
           ...
           <!--  <td><? echo $row->given_name ." ". $row->surname ?></td> -->
            ...
            <td>
              <form id="site_manager_submitted_form<?=$row->id?>" method="post" action="site_visit_complete_checklist.php?udid=<?=$row->dates?>" >
                <input type="hidden"  name="site_visit_submitted_form" value="<?=$row->dates?>" /> 
