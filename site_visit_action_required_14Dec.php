<?
  ...
  $get_data = $conn->query("select *,tbl_sm_report_main.id, tbl_sm_report_action.id as aid,tbl_sm_report_action.row_id as final_row_id, date(tbl_sm_report_filled.created) as created_filled FROM tbl_sm_report_main  join tbl_sm_report_action on tbl_sm_report_main.id = tbl_sm_report_action.row_id   join tbl_sm_report_filled on tbl_sm_report_action.checklist_udid  = tbl_sm_report_filled.checklist_udid  AND tbl_sm_report_filled.project_id ='3'  AND tbl_sm_report_filled.is_deleted = '0' AND tbl_sm_report_filled.is_submitted = '1' AND  (tbl_sm_report_action.actual_date  = '' or tbl_sm_report_action.actual_date IS  NULL) AND tbl_sm_report_action.action = '2' WHERE  tbl_sm_report_action.emp_id = '1'  order by  tbl_sm_report_main.task_number");
  ...
  <div class="main_form_header_blue" style="padding-left: 0;padding-right: 0;padding-top: 5px;height: 25%">
    <div class="row">
      <div class="col-sm-12">
        ...
        <div class="col-md-12" >
          <div class="tab text-center"><button class="tablinks" id="defaultOpen" style="float: none">Action Required<!-- <span class="badge" style="background: red;padding: 4px 0px;font-size: 14px;margin-top:-30px;margin-left: 5px"><? if($count_action->count) {?><div style="height: 0px;"></div><? }?></span> --></button></div>
        </div>
      </div>
    </div>
        		  ...
        		  <!--Pop Up for checklist editing-->
    				  <div class="modal fade" id="action_required_modal<?=$row_get_data->id?>" role="dialog">
                        ...
                        <textarea id="actualComment_<?=$row_get_data->id?>" rows="4" style="height: 70px; width: 100% ; border:none; outline: none;resize: none;background-color:#fff !important;border-radius: 10px; padding: 6px 12px;" required></textarea> 
                  		    ...
                  		    <? //}?> 
                          <input type="text" class="form-control form-control-left-radius" name="" id="empId_<?=$row_get_data->id?>" value="<?=$get_resp_email->email_add?>" readonly>
              				  <!-- </select> -->
            				  </div>
            				  <div class="" style="margin-top: 10px">
  ...
  <script>
    function validate(date,comm)
    {
      ...
      if(valid)
      {
  		  $.ajax({
          type: "POST",
          url: "ajax_site_manager_action_required.php",
