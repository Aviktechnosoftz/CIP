<?php
  ...
  if($id != "")
  {
    ...
    $capture_query= $conn->query("select tbl_site_attendace.id, employer_id, tbl_site_attendace.project_id,   tbl_site_attendace.today_date, IF( whom =0, tbl_site_attendace.induction_no, tbl_site_attendace.whom ) AS induction_number, whom FROM  `tbl_site_attendace` LEFT JOIN  `tbl_employee` ON IF( whom =0, tbl_site_attendace.induction_no = tbl_employee.id, tbl_site_attendace.whom = tbl_employee.id ) where tbl_site_attendace.id='".$id."' ORDER BY today_date DESC")->fetch_object();
      ...
      <div class="col-md-12">
            ...
            <div class="form-group">
              ...
              <div class="col-md-12 zeropadding">
                <div class="input-group">
                  ...
                  <input name="zip" id="no_worker" placeholder="No. of Workers" class="form-control"  type="text"  onKeyPress="edValueKeyPress()" onKeyUp="edValueKeyPress()" value="<? echo $get_query->no_of_worker ?>" >
                  <script type="text/javascript">
                    function edValueKeyPress()
                 {
        var edValue = document.getElementById("no_worker");
        var s = edValue.value;
    
        var lblValue = document.getElementById("run");
        ...
        //$("#lblValue").text(s);    
            ...
            <div class="form-group">
              <div class="col-md-12 label-position">
                <div id="text_below">
                  <i><? echo $get_query->company_name ?>&nbsp;has:&nbsp; <i id="run"><?echo $get_query->no_of_worker?> </i> &nbsp;Men Onsite<? echo "\r\n" ?><? echo $get_query->trade ?></i>
                </div>  
              </div>
            </div>
            <div class="form-group">
              ...
              <div class="col-md-12 zeropadding">    
                <textarea class="form-control worker-location" name="comment" id="loc" placeholder="3 men working in the main warehouse pouring slab 273 men digging the stormwater trench to main carpark 2 men installing bollards to doorways D38 & D36" ><? echo $get_query->employees_location  ?></textarea>
              </div>
            </div>
            <script>
              function imgDescCheck()
              {
                if(($('#upload_1').attr('src') != 'image/upload_image.png' && $('#desc1').val() == '') ||
                   ($('#upload_2').attr('src') != 'image/upload_image.png' && $('#desc2').val() == '') ||
                   ($('#upload_3').attr('src') != 'image/upload_image.png' && $('#desc3').val() == '') ||
                   ($('#upload_4').attr('src') != 'image/upload_image.png' && $('#desc4').val() == '') ||
                   ($('#upload_5').attr('src') != 'image/upload_image.png' && $('#desc5').val() == '') ||
                   ($('#upload_6').attr('src') != 'image/upload_image.png' && $('#desc6').val() == '') )
                {
            ...
            <Script>
              $(document).ready(function(){
                if($('#desc1').val() == '')
                { 
                  $('#desc1').removeAttr('readonly');
                }
                if( $('#desc2').val() == '' )
                {
                  $('#desc2').removeAttr('readonly');
                }
                if( $('#desc3').val() == '')
                {
                  $('#desc3').removeAttr('readonly');
                }
                if($('#desc4').val() == '')
                {
                  $('#desc4').removeAttr('readonly');
                }
                if($('#desc5').val() == '')
                {
                  $('#desc5').removeAttr('readonly');
                }
                if($('#desc6').val() == '')
                {
                  $('#desc6').removeAttr('readonly');
                }
              });
            </Script>
            <div class="form-group">
              <div class="col-md-2 text-left" style="">
                ...
                <!--upload document-->
                <input type="file" name="photo[]" id="file_1" onchange="document.getElementById('upload_1').src = window.URL.createObjectURL(this.files[0]);$('#viewer1').show();">
                <!--Delete Button-->
                ...
                <input name="desc1" placeholder="Image Description" class="simple_field form-control"  id="desc1"  type="text" value="<? if($get_query->image_1_text != "" ) {echo $get_query->image_1_text; }else{echo '';} ?>" readonly>
              </div>                    
              <div class="col-md-2 text-center" style="padding-left: 0px">
                ...
                <input type="file" name="photo[]" id="file_2" onchange="document.getElementById('upload_2').src = window.URL.createObjectURL(this.files[0]);$('#viewer2').show();">
                <input type="button"  id="saveForm" onclick="delete_image(2)" style="background-image: url(image/icon_delete.png);" class="btn"/>
                <input name="desc2" placeholder="Image Description" class="simple_field form-control"  id="desc2"  type="text" value="<? if($get_query->image_2_text != "" ) {echo $get_query->image_2_text; }else{echo '';} ?>" readonly>
              </div>
              <div class="col-md-2 text-center" style="padding-left: 0px">
                ...
                <input type="file" name="photo[]" id="file_3" onchange="document.getElementById('upload_3').src = window.URL.createObjectURL(this.files[0]);$('#viewer3').show();">
                <input type="button"  id="saveForm" onclick="delete_image(3)" style="background-image: url(image/icon_delete.png);" class="btn"/>
                <input name="desc3" placeholder="Image Description" class="simple_field form-control"  id="desc3"  type="text" value="<? if($get_query->image_3_text != "" ) {echo $get_query->image_3_text; }else{echo '';}?>" readonly>
              </div>
              <div class="col-md-2 text-center" style="padding-left: 0px">
                ...
                <input type="file" name="photo[]" id="file_4" onchange="document.getElementById('upload_4').src = window.URL.createObjectURL(this.files[0]); $('#viewer4').show();">
                <input type="button"  id="saveForm" onclick="delete_image(4)" style="background-image: url(image/icon_delete.png);" class="btn"/>
                <input name="desc4" placeholder="Image Description" class="simple_field form-control"  id="desc4"  type="text" value="<? if($get_query->image_4_text != "" ) {echo $get_query->image_4_text; }else{echo '';}?>" readonly>
              </div>
              <div class="col-md-2 text-center" style="padding-left: 0px">
                ...
                <input type="file" name="photo[]" id="file_5" onchange="document.getElementById('upload_5').src = window.URL.createObjectURL(this.files[0]); $('#viewer5').show();">
                <input type="button"  id="saveForm"  onclick="delete_image(5)"  style="background-image: url(image/icon_delete.png);" class="btn"/>
                <input name="desc5" placeholder="Image Description" class="simple_field form-control"  id="desc5"  type="text" value="<? if($get_query->image_5_text != "" ) {echo $get_query->image_5_text; }else{echo '';}?>" readonly>
              </div>
              <div class="col-md-2 text-right" style="padding-left: 0px;width:15%;">
                ...
                <input type="file" name="photo[]" id="file_6" onchange="document.getElementById('upload_6').src = window.URL.createObjectURL(this.files[0]); $('#viewer6').show();">
                <input type="button"  id="saveForm" onclick="delete_image(6)" style="background-image: url(image/icon_delete.png);" class="btn"/>
                <input name="desc6" placeholder="Image Description" class="simple_field form-control"  id="desc6"  type="text" value="<? if($get_query->image_6_text != "" ) {echo $get_query->image_6_text; }else{echo '';}?>" readonly>
              </div>
            </div>   
            <script>
              function onerrorfun(id) 
              {
                ...
                $('#upload_'+id).attr('alt', 'Document Uploded Here');
              }
            </script>
            ...
            <div class="form-group">
              <div class="col-md-12  text-center">
                <div class="col-md-4 col-md-offset-2">
                  <input type="button" class="btn btn-danger form_submit_button" value="Save Changes" name="save" onclick="update()">
                </div>
                ...
                <input type="hidden" id="attendance_id" name="atendance_id" value="<? echo $_REQUEST['attendance_form']?>">
                <div class="col-md-4">   
                  <input type="submit" class="btn btn-success form_submit_button" value="Show Image Gallery" id="show"  name="show">
                  <script>
              ...
              function update()
                    {
                 var v = imgDescCheck();
                      var worker = $('#no_worker').val().trim();
                       var location = $('#loc').val().trim(); 
                      if(worker=''|| worker==0 || !$.isNumeric(worker))
                       {
                        alert('Please Enter number of Workers Accept only numeric value.');
                        return false;
                       }
                      else if(location=='')
                       {
                  ...
                  function delete_image(id)
                  {
                    ...
                    // alert("ok");
                    $('#viewer'+id).hide();
                    $('#upload_'+id).css('content', 'url(image/upload_image.png)');
                    $('#desc'+id).removeAttr('readonly');
                ...
                <script type="text/javascript">
                  $(document).ready(function(){
                    ...
                    if (img1.src.split("/").reverse()[0] == "upload_image.png" || img1.src == "")
                    { 
                      $("#viewer1").hide();
                    }
                    else
                    ...
                    if (img2.src.split("/").reverse()[0] == "upload_image.png" || img2.src == "")
                    {
                      $("#viewer2").hide();
                    } 
                    else  
                    ...
                    if (img3.src.split("/").reverse()[0] == "upload_image.png" || img3.src == "")
                    { 
                      $("#viewer3").hide();
                    } 
                    else  
                    ...
                    if (img4.src.split("/").reverse()[0] == "upload_image.png" || img4.src == "")
                    { 
                      $("#viewer4").hide();
                    } 
                    else  
                    ...
                    if (img5.src.split("/").reverse()[0] == "upload_image.png" || img5.src == "")
                    { 
                      $("#viewer5").hide();
                    } 
                    else  
                    ...
                    if (img6.src.split("/").reverse()[0] == "upload_image.png" || img6.src == "")
                    { 
                      $("#viewer6").hide();
                    } 
                    else  
