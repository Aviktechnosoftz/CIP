...
<div class="main_form_header">
  <div class="col-md-12" style="padding-top: 14px">
    <div class="col-md-6 main_title_container">
        ...
        <label class="control-label rightpadding"><span class="form-name">SITE ATTENDANCE REGISTER MONTHLY REPORT&nbsp;<?// echo $project_name->project_name ?> </span></label>
...
<div class="Main_Form_list">
      ...
      <div class="form-group">
        ...
        <table  class="table table-hover" style= "border: 1px solid rgba(128, 128, 128, 0.57);"">
          <div class="col-sm-12" style="padding-left: 0">The Requested Site Attendance Report for the Month: &nbsp<span style="color: red;font-weight: bold"> <?=$month_name?> </span></div>
          <thead>
                  ...
                  <td><? echo $t ?></td> <? $counter++;
                    ...
                    {if ($check < 10) {
                       $check = str_pad($check, 2, "0", STR_PAD_LEFT);
                        ...
                        $date= substr($row1['today_date'], -2);

                      if($check == $date)
                      { $z = $check ;
                          //echo '<td><a href="site_attendance_approved.php?redirect=true&&d='.$row1["today_date"].'&e='.$row1["today_date"].'</a>'.$row1["sum"].'</td>';
?>
                          <td><a href="site_attendance_approved.php?redirect=true&d=<?=$row1['today_date']?>&e=<?=$emp1?>"><?=$row1['sum']?></a></td>
                          <?
                            $check++;
                          break;
                      }
