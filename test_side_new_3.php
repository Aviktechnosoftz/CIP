...
// $_SESSION['menu'] = $page_name;

// }
// else
// {
//   echo " ok";
// $page_name= $_SESSION['menu'];
// }
// print_r($_SESSION);

// echo "select * 
// from (
//     select id,href,'parent_menu' as 'tab'  from parent_menu
//     union all
//     select  id,href ,'sub_menu' as 'tab' from sub_menu
//     union all
//     select  id,href ,'sub_sub_menu' as 'tab' from sub_sub_menu
// ) a
// where href = '".$_SESSION['menu']."'";


$page_name= basename($_SERVER['PHP_SELF']);

$get_table_name =$conn->query("select * 
from (
    select id,href,'parent_menu' as 'tab'  from parent_menu
    union all
    select  id,href ,'sub_menu' as 'tab' from sub_menu
...
where href = '".$page_name."'")->fetch_object();


// print_r($_SESSION['menu']);
//echo "Select * from sub_menu where parent_id=(Select id from parent_menu where name='".$_SESSION['menu']."')";

$id="parent_id";
if($get_table_name->tab=="parent_menu")
{
  $id="id";
}
// echo "Select * from sub_menu where parent_id=(Select main_parent from ".$get_table_name->tab."  where href='".$page_name."')";
$get_sub_menu= $conn->query("Select * from sub_menu where parent_id=(Select main_parent from ".$get_table_name->tab."  where href='".$page_name."')");
...
<div id="sidebar" data-position="left">
 <div style="  "> 
  ...
  <div class="contents">
      ...
      while ($row_sub_menu=$get_sub_menu->fetch_object()){ 

      ?>
    <div class="content show" id="contents_show" data-content="content<?=$row_sub_menu->id ?>">
    ...
    padding-bottom: 1%;" class="sub_content">
      ...
      <div class="icon" data-content="content<?=$row_sub_menu->id ?>" style="    float: left;"> 
      <img src="image/site2.png" style=" width: 4vh;"> 
      </div>
      <h4 style="color:white;font-size: 2vh;    margin-top: 0vh;
    margin-bottom: 0px;  
    padding-top: 1vh;
    padding-bottom: 1vh;
    ...
    " ><a  style="color: white;text-decoration: none;cursor: pointer;" <?if($row_sub_menu->href!="")echo "href=".$row_sub_menu->href  ?> ><?= $row_sub_menu->name ?></a></h4>
    ...
    margin-left: 6vw;"></div>
      ...
      <ul class="list">
      <?$get_sub_sub_menu=$conn->query("Select name,href from sub_sub_menu where parent_id=".$row_sub_menu->id.""); 
      
      if($get_sub_sub_menu->num_rows >0)
      {

        while($row_sub_sub_menu=$get_sub_sub_menu->fetch_object())
        {
          if($row_sub_sub_menu->name !="")
          {
         
    ?>
        <li><a  style="color: white;text-decoration: none;cursor: pointer;" <?if($row_sub_sub_menu->href!="")echo "href=".$row_sub_sub_menu->href  ?> ><?=$row_sub_sub_menu->name?></a></li>
