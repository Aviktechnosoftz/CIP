<? 
  ...
  foreach ($json['driver'] as $key_) 
  {
   ...
   $count++;
        ...
        $year = date("Y");
        $date = new DateTime();
        $weeks = array_column($list_week, 'week_count');
		...
		{ 
          if(value['week_count'] == week_count)
            ...
            if(week_count == curr_week){
			  ...
			  $("#curr_w_s").html(firstDayOfWeek_ws('2017',week_count));
            ...
            //  $("#curr_rank").html(get_index(value['score'],get_array(week_count,'score'))+" of "+ total);
			  ...
			  value['id'] = get_index(value['score'],get_array(week_count,'score'));
...
<div class="page-header"  id="title" style="min-height: 95vh;">
  ...
  <p align="center" id="datamsg" style="display: none;color: red;font-weight: 600;">There is no data of this week</p>
...
<input type="button" onclick="$('#history_graph').show();$('#history').hide();" style="margin-top: 2vh;outline: none;
  ...
  <input type="button" onclick="$('#rank_graph').show();$('#history').hide();" style="margin-top: 2vh;outline: none;
  border:none;
  background-size: 100%;
  background: url('image/Graph_Unpressed.png');
  background-position: top center;
...
++$counter_week;
$calc= ($data['score']/$weeks[$counter_week])*100;
 $date->setISODate($year,$data['week_count']);
 $date_data=$date->format('m-d-y');
