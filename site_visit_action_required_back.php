...
// print_r($_SESSION);
  $get_data_1 = $conn->query("select *,tbl_sm_report_main.id,tbl_sm_report_action.id as act_id,date(tbl_sm_report_main.created) as filled_created FROM tbl_sm_report_main  join tbl_sm_report_action on tbl_sm_report_main.id = tbl_sm_report_action.row_id   join tbl_sm_report_filled on tbl_sm_report_action.checklist_udid  = tbl_sm_report_filled.checklist_udid  AND tbl_sm_report_filled.project_id ='".$_SESSION['admin']."'  AND tbl_sm_report_filled.is_deleted = '0' AND tbl_sm_report_filled.is_submitted = '1' AND  (tbl_sm_report_action.actual_date  = '' or tbl_sm_report_action.actual_date IS  NULL) AND tbl_sm_report_action.action = '2' WHERE  tbl_sm_report_action.emp_id = '".$_SESSION['induction']."'  order by  tbl_sm_report_main.task_number");

  $get_data_2=$conn->query("select *,tbl_sm_report_task_obs.id,tbl_sm_report_action.id as act_id  FROM tbl_sm_report_task_obs  join tbl_sm_report_action on tbl_sm_report_task_obs.id = tbl_sm_report_action.row_id   join tbl_sm_report_filled on tbl_sm_report_action.checklist_udid  = tbl_sm_report_filled.checklist_udid  AND tbl_sm_report_filled.project_id ='".$_SESSION['admin']."'  AND tbl_sm_report_filled.is_deleted = '0' AND tbl_sm_report_filled.is_submitted = '1' AND  (tbl_sm_report_action.actual_date  = '' or tbl_sm_report_action.actual_date IS  NULL) AND tbl_sm_report_action.action = '2' WHERE  tbl_sm_report_action.emp_id = '".$_SESSION['induction']."'  order by  tbl_sm_report_task_obs.task_number");



  $get_count = $conn->query("select count(*) as count FROM tbl_sm_report_main  join tbl_sm_report_action on tbl_sm_report_main.id = tbl_sm_report_action.row_id   join tbl_sm_report_filled on tbl_sm_report_action.checklist_udid  = tbl_sm_report_filled.checklist_udid  AND tbl_sm_report_filled.project_id ='3'  AND tbl_sm_report_filled.is_deleted = '0' AND tbl_sm_report_filled.is_submitted = '1' AND  (tbl_sm_report_action.actual_date  = '' or tbl_sm_report_action.actual_date IS  NULL) AND tbl_sm_report_action.action = '2' WHERE  tbl_sm_report_action.emp_id = '1'  order by  tbl_sm_report_main.task_number")->fetch_object();
?>
<header>
  <? include('header.php'); ?>
      ...
      <fieldset>
      	...
      	<div class="form-group">
        	<div class="col-sm-12" style="padding-right: 0px;padding-left: 0px">
        		<? 
            	while($row_get_data = $get_data_1->fetch_object() OR $row_get_data_task=$get_data_2->fetch_object())
          		{
          	?>
          	...
          	<div id="London" class="tabcontent" >
	            <table id="mytable" class="table table-bordred" style=" background-color: #333240;color: #fff;">
              	<tbody>
	                <tr class="content_tr" id="mod_button" data-toggle="modal" data-target="#action_required_modal<? if($row_get_data->task_number !="" OR $row_get_data->task_number !=NULL) { echo $row_get_data->act_id;} else
                    echo $row_get_data_task->act_id;?>">
                  	<td style="text-align: left;vertical-align: middle;padding-left: 30px"  class="col-sm-1"><? if($row_get_data->task_number !="" OR $row_get_data->task_number !=NULL) { echo $row_get_data->task_number;} else
                    echo $row_get_data_task->task_number;?></td>    
                  	<td class="col-sm-9" style="vertical-align: middle;padding-left: 30px"><?=$row_get_data->task_desc?>
                    	<div id="span2" class="t r" style="vertical-align: middle;">
                      	...
                      	<b>Action Required:</b>&nbsp;<? echo $row_get_data->action_required; ?>
        		...
        		<!--Pop Up for checklist editing-->
    				<div class="modal fade" id="action_required_modal<? if($row_get_data->task_number !="" OR $row_get_data->task_number !=NULL) { echo $row_get_data->act_id;} else
                    echo $row_get_data_task->act_id;?>" role="dialog">
        				...
        				<div class="modal-content" style="background: #393848 !important;">
          				...
          				<div class="modal-body">
            				...
            				<div class="" style="margin-top: 10px">
              				...
              				<textarea type="text" name="" id="actionRequired" style="height: 70px; width: 100% ; border:none; outline: none;resize: none;background-color:#fff !important;border-radius: 10px; padding: 6px 12px;"><?=$row_get_data->action_required;?></textarea>
                    ...
                    <input type="hidden" id="empid" value="<?=$row_get_data->emp_id?>">
          				...
          				<div class="modal-footer" style="border-top: none;">
            				<button type="button" class="btn btn-default"  style="background-color:#f47821 ;margin-top:3rem;outline: none;border-radius:6vh;outline: none;border:none;color: #fff;padding-left: 25px;padding-right: 25px" onclick="update_action_required(<?if($row_get_data->task_number !="" OR $row_get_data->task_number !=NULL) { echo $row_get_data->act_id;} else
                    echo $row_get_data_task->act_id;?>,$('#actualDate_<?=$row_get_data->id?>').val(),$('#actualComment_<?=$row_get_data->id?>').val(),$('#empid').val(),'<?=$row_get_data->final_row_id ?>','<?=$row_get_data->created_filled ?>')">Save</button>
...
<script>
  function validate(date,comm)
  {
	...
	{
    alert(id+" "+date+" "+comm+" "+empid+" "+row_id+" "+checklist_date);
