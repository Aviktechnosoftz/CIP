...
$exjson = json_decode($JsonData);
 ...
 $cur_date= strtotime(date('Y-m-d'));
  ...
  $udid=md5($sum);
	...
	$check_exist= $conn->query("Select * from tbl_filled_sm_weekly where created= DATE('".$exjson->created."') AND project_id='".$exjson->project_id."'")->num_rows;
	 if($check_exist > 0)
	 {
...

	 {
	 	  ...
	 	  $get_inserted =  $conn->query("select * from tbl_filled_sm_weekly where checklist_udid='".$udid."' ");
						 ...
						 $result = count($rows_tbl_filled_sm);
						 	if($result > 0)
						 	{
	 	 				 	echo "{\"response\":\"200\",\"datasmfilled\":".json_encode($rows_tbl_filled_sm[0])."}";
	 	  					}
	 	  					else
	 	  					{
