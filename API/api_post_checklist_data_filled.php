...
foreach ($exjson['checklistdatafilled'] as $row) 
{  

	if($row['id'] < 0)
	{
				$count ++;
				$new_id = "select max(id) as idnew From tbl_checklist_data_filled";
...
{  
	...
	if ($row['id'] < 0 )
	{
      ...
      $query2 = "insert into tbl_checklist_data_filled set `id` = '".$newID."',
	  row_id = '". $row['row_id']."',
	  assign_checklist_uuid = '". $row['assign_checklist_uuid']."',
	  filled_by = '". $row['filled_by']."',
	  compliance = '". $row['compliance']."',
	  comments = '". $row['comments']."',
	  acceptance_criteria_Data = '". $row['acceptance_criteria_Data']."',
	...
	{
		    $sql_1 = " update tbl_checklist_data_filled set 
					   row_id = '". $row['row_id']."',
					  assign_checklist_uuid = '". $row['assign_checklist_uuid']."',
					  filled_by = '". $row['filled_by']."',
					  compliance = '". $row['compliance']."',
