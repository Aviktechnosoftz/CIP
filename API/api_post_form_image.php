...
foreach ($exjson['formimage'] as $row) 
{  

	if($row['id'] < 0)
	{
				$count ++;
				$new_id = "select max(id) as idnew From tbl_form_image";
...
{  
	 ...
	 if ($row['id'] < 0 )
	{
      ...
      $query2 = "insert into tbl_form_image set `id` = '".$newID."',
	  row_id = '". $row['row_id']."',
	...
	{
		    $sql_1 = "update tbl_form_image set 
					  row_id = '". $row['row_id']."',
					  image_name = '". $row['image_name']."',
					  ref_doc_id = '". $row['ref_doc_id']."',
					  assign_uuid = '". $row['assign_uuid']."',
