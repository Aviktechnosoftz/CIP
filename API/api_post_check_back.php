...
{  
  ...
  if($row['id'] < 0)
  {
        ...
        $maxid_fpr = $maxid_acc + $countcpi_fpr;
...
{  
  ...
  `last_inspection_weekly`  ='".$row['inspected_by']."',
...
foreach ( $exjson['tbl_fpr_filled'] as $row )
{  
...
if($result2 OR $result2_ncic OR $result2_acc OR  $result2_fpr) // success 
{
	mysql_query("COMMIT");
	createJson_etr($ids,$etr_udid);
	createJson_ncic($ids_ncic,$ncic_udid);
	createJson_acc($ids_acc,$acc_udid);
  createJson_acc($ids_fpr,$fpr_udid);
	
}
else
{
	 mysql_query("ROLLBACK");
	 echo "{\"responce\":\"201\",\"data\":\"".$result2.$result2_acc.$result2_ncic.$result2_fpr."\"}";
...
function getID($oldid,$arrayIds)
{
  ...
  function createJson_fpr($arrayIds,$fpr_udid)
 {
  ...
  foreach ($arrayIds as $value) 
   ...
   echo "\"data_wc_fpr_task_obs\": ".json_encode($fpr_udid).",\"data_wc_fpr_filled\": [".$message ."],";
