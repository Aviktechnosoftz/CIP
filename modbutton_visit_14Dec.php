...
$count_action = $conn->query("select count(*) as count FROM tbl_sm_report_main  join tbl_sm_report_action on tbl_sm_report_main.id = tbl_sm_report_action.row_id   join tbl_sm_report_filled on tbl_sm_report_action.checklist_udid  = tbl_sm_report_filled.checklist_udid  AND tbl_sm_report_filled.project_id ='".$_SESSION['admin']."'  AND tbl_sm_report_filled.is_deleted = '0' AND tbl_sm_report_filled.is_submitted = '1' AND  (tbl_sm_report_action.actual_date  = '' or tbl_sm_report_action.actual_date IS  NULL) AND tbl_sm_report_action.action = '2' WHERE  tbl_sm_report_action.emp_id = '".$_SESSION['induction']."'  order by  tbl_sm_report_main.task_number")->fetch_object();
?>
